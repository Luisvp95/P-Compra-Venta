...
class HomeController extends Controller
{
    ...
    {   
        ...
        $ventasdia = DB::select('SELECT DATE_FORMAT(v.sale_date, "%d/%m/%y") AS dia, SUM(v.total) AS totaldia 
        FROM sales v WHERE v.status="VALID" GROUP BY dia ORDER BY v.sale_date DESC LIMIT 15');

        $totales=DB::select('SELECT 
        (select ifnull(sum(c.total), 0) from purchases c 
        where DATE(c.purchase_date)=curdate() and c.status="VALID") as totalcompra, 
        (select ifnull(sum(v.total), 0) from sales v 
        where DATE(v.sale_date)=curdate() and v.status="VALID") as totalventa');
